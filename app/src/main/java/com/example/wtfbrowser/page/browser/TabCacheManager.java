...
import com.example.wtfbrowser.page.tab.NewTabFragmentV2;
...
public class TabCacheManager implements IBrowser.TabController {
    ...
    private LruCache<TabInfo, Fragment> lruCache;   //缓存页面 - LinkedHashMap
    private final List<TabInfo> infoList = new ArrayList<>();   //页面信息记录
     ...
     *
     * 如所有标签页都被关闭，则新建一个标签页
     * 如关闭了第一个标签页，显示新的第一个标签页
     * 如关闭标签页位置在List中间，显示前一位置的标签页
     * @param info
     */
